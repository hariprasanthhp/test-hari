...
SysGroups:any = [];
migrationTypeArray:any = [];
  systemGroupList: any;
  deviceGroup: any;
  deviceCount: any;
constructor(private titleService: Title,private translateService: TranslateService,private modalService: NgbModal,
  private dialogService: NgbModal,    private service:ProfileService,private sso: SsoAuthService,  private commonOrgService: CommonService,private router: Router,private Foundationservice: FoundationManageService,private downloadService: DownloadService, private http: HttpClient,) {
...
ngOnInit(): void {
  ...
  this.languageSubject = this.translateService.selectedLanguage.subscribe((data: any) => {
    this.language = data;
    this.titleService.setTitle(`${this.language['Jobs']} - ${this.language['Migrations']} - ${this.language['Settings']} - ${this.language['Configuration']} - ${this.language['Deployment']} - ${this.language['Calix Cloud']}`);
  })
  this.titleService.setTitle(`${this.language['Jobs']} - ${this.language['Migrations']} - ${this.language['Settings']} - ${this.language['Configuration']} - ${this.language['Deployment']} - ${this.language['Calix Cloud']}`);
  this.dtOptions = {
...
GotoProfile(){
this.router.navigate(["/cco-foundation/foundation-configuration/configuration-settings/migrations-rg/profiles"])
}
Gotomigration(id,name){
this.router.navigate(["/cco-foundation/foundation-configuration/configuration-settings/migrations-rg/migration-mapping"],{ queryParams:{migrationId:id,migrationName:name}})
}
...
this.dtOptions = {
  ...
  ajax: (dataTablesParameters: any, callback) => {
    ...
    if(dataTablesParameters?.order[0]?.column == 4) {
      params  = {"filter":`${this.searchtext || ''}`,"page": `${pageNumber }`,"size": pageSize,bFilterRGOnly: true,sort:`dateCompleted,${this.sortedColumnDetails}`}
    }
    else if(this.sortedColumnDetails && dataTablesParameters?.order[0]?.column == 0) params = {"filter":`${this.searchtext || ''}`,"page": `${pageNumber }`,"size": pageSize,bFilterRGOnly: true,sort:`migrationName,${this.sortedColumnDetails}`}
    else  params = {"filter":`${this.searchtext || ''}`,page: `${pageNumber }`,"size": pageSize ,bFilterRGOnly: true}
      ...
      ).subscribe((resp: any) => 
      {
        ...
        console.log("jobList",resp?.result.filter((x)=> x?.type === 'RG'))
...
if(data){
 ...
 this.JobMigForm.patchValue({name:data?.migrationName,description:data?.description,deviceType: data?.type, group:data?.rgSystemGroupName})
 ...
 this.dialogService.open(modal, { size: 'xl', centered: true, backdrop: 'static', windowClass: 'add-user-modal' });
  ...
  this.JobMigForm.patchValue({name:'',deviceType: 'RG',group:'',description:''})
  this.dialogService.open(modal, { size: 'xl', centered: true,backdrop: 'static', windowClass: 'add-user-modal' });
}

};
onSubmit(){
  console.log("JobMigForm",this.JobMigForm.value)
...
if(this.editJobMig){
  this.service.updateJobMig(this.migrationId,this.JobMigForm.value.name,this.JobMigForm.value.description,this.JobMigForm.value.deviceType).subscribe((res: any) => {
    ...
    this.modelloader=false
    this.pageErrorHandle(err);
    this.JobMigForm.patchValue({name:'',deviceType:'RG',group:'',file:'',description:''})
    this.dialogService.dismissAll();
  })
}else{
  let staticGrp = this.systemGroupList.filter(data =>{
...
close(): void {
this.JobMigForm.patchValue({name:'',description:'',deviceType:'RG',group:''})
this.dialogService.dismissAll();
}
closeAlert() {
this.error = false;
...
getDeviceGoupList(count?) {
  this.loader=true
  this.deviceGroup = this.Foundationservice.getDeviceGoupList(this.sso.getOrgId(), this.count).subscribe((data: any) => {
    this.loader=false
    this.systemGroupList = data ? data : [];
    console.log("systemGroupList",this.systemGroupList)
    }, (err: HttpErrorResponse) => {
    this.loader=false
    this.pageErrorHandle(err);
  })
