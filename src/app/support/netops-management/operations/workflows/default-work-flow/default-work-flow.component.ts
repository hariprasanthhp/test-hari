...
@Component({
  selector: 'app-default-work-flow',
  templateUrl: './default-work-flow.component.html',
  styleUrls: ['./default-work-flow.component.scss']
})
export class DefaultWorkFlowComponent implements OnInit {
  ...
  language: any;
  languageSubject;
  inputOperationType = [{ name: 'Time Scheduler', id: 'Time Scheduler' }]
  ...
  customEndRange: any = null;
  startDateTime;
  diabledTrigger: boolean = true;
  weekselected = []
  daysOfMonth = 1;
  rangeOccurrence = null;
  daily
  weekly
  monthly
  endBy = false
  ...
  editItem_id: string;
  constructor(
    private translateService: TranslateService, private api: NetopsServiceService,
    private dateUtils: DateUtilsService, private sso: SsoAuthService, private route: ActivatedRoute,
    private router: Router, private commonOrgService: CommonService,
  ) { }
  ...
  go_next() {
    ...
    if (this.scheduleValueSelected === 'Time Scheduler') {
      ...
      if (this.RangeRecurrenceEndBy) {
        ...
        this.workflowInputData['execPolicy']['window']['recurrence'] = this.rangeOccurrence ? this.rangeOccurrence : null;
      }
    }
    this.workflowInputData['bPriorNewAndFailed'] = true
    if (this.editItem_id) {
      delete this.workflowInputData?.orgId;
      this.api.editWorkflowData(this.workflowInputData).subscribe(res => {
      ...
      this.api.AddWorkflowData(this.workflowInputData).subscribe(res => {
        this.sso.redirectByUrlForWorkflow([
          `/support/netops-management/operations/workflows`,
          `/cco/operations/cco-system-operations/workflows`,
          `./cco-foundation/foundation-configuration/configuration-workflow/workflows`,
  ...
  getWorkflowById() {
    this.api.getWorkflowById(this.editItem_id).subscribe((res: any) => {
      if (res) {
        ...
        this.timeWindow = res?.execPolicy?.window?.windowLength ? res?.execPolicy?.window?.windowLength / 60 : 60;
