...
export class MonthlyUsageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.sso.setActionLog('CSC', 'pageHit', 'reports - monthly usage', window.location.href, 'monthly usage reports intiated');
  }
  ...
  loadChartData(data: any) {
    ...
    this.subscribe = this.faNetworkApiService.getMonthlyUsage(data, endpointId).subscribe((res: any) => {
      ...
      console.log(this.trafficChartOptions);
