...
@Component({
  selector: 'app-triggered-campaigns',
  templateUrl: './triggered-campaigns.component.html',
  styleUrls: ['./triggered-campaigns.component.scss']
})
export class TriggeredCampaignsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.languageSubject = this.translateService.selectedLanguage.subscribe(data => {
      this.language = data;
      this.titleService.setTitle(`${this.language["Triggered Campaigns"]}-${this.language["Marketing_Cloud"]}-${this.language["Calix Cloud"]}`);
...
dropDownListValue(){
  this.finalStatusDataarr = [
    ...
    { id: 3, name: this.language.Active},
    { id: 3, name: this.language.Paused},
  ];
  ...
  campaignChannelList(ref?: any) {
    ...
    if (this.scopes.campaignRead) {
      ...
      this.campaignListSubject = this.marketingCampaignsApiService.CampaignsListGET('Triggered',lang).subscribe((res: any) => {
        // res = []
        console.log(res)
  ...
  keyEvent(event: KeyboardEvent) {
    ...
    if (!isClose && event.key === 'Enter' && this.deleteListCampaign == 'delete') {
      this.campaignDelete(0);
    }else if (!isClose && event.key === 'Enter' && this.unpausecampaign == 'Paused') {
      this.pauseUnPause('Paused');
    }else if (!isClose && event.key === 'Enter' && this.pausecampaign == 'Active') {
      this.pauseUnPause('Active');
    }else  if (!isClose && event.key === 'Enter' && this.resetListCampaign == 'reset') {
      this.campaignDelete(1);
    }
  ...
  campaignResetPrompt(data) {
    this.deleteCampaignObject = data;
    this.dialogService.open(this.resetCampaign, { backdrop: "static", keyboard: false, size: 'lg', centered: true, windowClass: 'default-modal-ui modal-cust-md' });
    this.resetListCampaign = "reset";
  }

  pauseUnpausePrompt(data,status) {
    this.deleteCampaignObject = data;
    if(status == 'Active'){
      this.dialogService.open(this.pauseCampaign, { backdrop: "static", keyboard: false, size: 'lg', centered: true, windowClass: 'default-modal-ui modal-cust-md' });
      this.commonFunctionsService.trackPendoEvents('Engagement_Cloud','Campaign resumed');
      this.pausecampaign = "Active";
    }else{
      this.dialogService.open(this.unpauseCampaign, { backdrop: "static", keyboard: false, size: 'lg', centered: true, windowClass: 'default-modal-ui modal-cust-md' });
      this.commonFunctionsService.trackPendoEvents('Engagement_Cloud','Campaign paused');
      this.unpausecampaign = "Paused"; 
    }
  }

  pauseUnPause(status) {
    if (this.scopes.campaignWrite) {
      this.campaignListDeleteSubject = this.marketingCampaignsApiService.CampaignPauseUnpause(`${this.deleteCampaignObject.campaignId}`,status)
        .subscribe((res: any) => {
          setTimeout(() => {
            this.campaignChannelList(true);
          }, 1000);
  ...
  selectCampaign(data,from) {
    
    let datas = data.campaignType == 'Triggered' ? sessionStorage.setItem('triggered','new'): sessionStorage.removeItem('triggered')
    let fromData = (from == 0 && data.status == this.language.active) ? sessionStorage.setItem('StatusAct','Active'): sessionStorage.removeItem('StatusAct')
  ...
  selectCampaignFilter() {
    ...
    if (this.campaignType == this.language.All && this.campaignStatus == this.language.All) {
      ...
      this.marketingCampaignTable = this.FiltermarketingCampaign.filter(
        (x) =>{
          if(x.campaignType != null){
            return x.campaignType == this.campaignType
          }else{
            return x.segmentType  == this.campaignType
          }
         }
      );
console.log(this.marketingCampaignTable,'this.marketingCampaignTable===')
    }
    else {
      this.marketingCampaignTable = this.FiltermarketingCampaign.filter(
        (x) =>{
          if(x.campaignType != null){
            return x.campaignType == this.campaignType  && x.status == this.campaignStatus
          }else{
            return x.segmentType  == this.campaignType  && x.status == this.campaignStatus
          }
        }
      );
    }
