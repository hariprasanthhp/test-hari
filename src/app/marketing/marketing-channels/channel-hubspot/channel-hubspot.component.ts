...
@Component({
  selector: 'app-channel-hubspot',
  templateUrl: './channel-hubspot.component.html',
  styleUrls: ['./channel-hubspot.component.scss']
})
export class ChannelHubspotComponent implements OnInit {
  language: any;
  loading = false;
  languageSubject: any;
  ...
  hubspotResultsError: boolean = false;
  timeframes = [
  ...
  ngOnInit(): void {
    ...
    this.languageSubject = this.translateService.selectedLanguage.subscribe(data => {
      this.getViewResult(this.selectVal)
      this.language = data;
      this.titleService.setTitle(`${this.language["HubSpot_Marketing_Channel_Results"]}-${this.language["Marketing_Cloud"]}-${this.language["Calix Cloud"]}`);
  ...
  getViewResult(data) {
    ...
    this.http.get(`${environment.API_BASE_URL}cmc-mchannel/marketingChannel/results/Hubspot?months=` + data).subscribe((json: any) => {
      this.hubspotResultsError = false;
      this.mailchimpError = false;
