...
@Component({
  selector: 'app-channel-mobile',
  templateUrl: './channel-mobile.component.html',
  styleUrls: ['./channel-mobile.component.scss']
})
export class ChannelMobileComponent implements OnInit {
  language: any;
  languageSubject: any;
  ...
  mobileNotificationResultsError: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.languageSubject = this.translateService.selectedLanguage.subscribe(data => {
      this.getViewResult(this.selectVal)
      this.language = data;
      this.titleService.setTitle('Results - Mobile Notifications -  Marketing - Calix Cloud');
      this.titleService.setTitle(`${this.language["Command_IQ_Mobile_Notification_Results"]}-${this.language["Campaigns"]}-${this.language["Marketing_Cloud"]}-${this.language["Calix Cloud"]}`);
  ...
  getViewResult(data) {
    ...
    this.http.get(`${environment.API_BASE_URL}cmc-campaigns/campaign/mobileNotificationHistory?months=` + data).subscribe((json: any) => {
      this.mobileNotificationResultsError = false;
      this.mobileError = false;
      // setTimeout(() => {
      if (this.isRerender) {
        this.rerender();
        if (json) {
          this.channelResultTable = json
      ...
      if (json === null) {
        this.mobileNotificationResultsError = true;
      ...
      if (err.status === 500) {
        ...
        this.mobileErrorMsg = 'Error! Internal Server Error';
      }
      else if (err.status === 504 || err.status == 502) {
        this.mobileError = true;
        this.mobileErrorMsg = 'Error! Gateway Time-Out';
      }
  ...
  searchName(searchText) {
    ...
    //     x.segmentName.toLowerCase().includes(searchText.toLowerCase())
    //   );
    //   this.rerender();
