...
@Component({
  ...
  templateUrl: './service-provisioning.component.html',
  styleUrls: ['./service-provisioning.component.scss']
})
export class ServiceProvisioningComponent implements OnInit {
  ...
  selectedtype: string;
  provisionOntService:boolean=false;
  showAsmInformation:boolean=false;
  language;
  languageSubject;
  id;
  ...
  ORG_ID: string;
  orgData: any;
  enableSubscriberAssocition = false;
  updateSubs: any;
  infoBody: string;
  modalRef: any;
  enableSubscriberAssoc: boolean;
  infoTitle: string;
  constructor(private ccoOrgAdminService: CcoOrgAdminService,
  ...
  ngOnInit(): void {
    ...
    this.languageSubject = this.translateService.selectedLanguage.subscribe(data => {
      this.language = data;
      this.titleService.setTitle(`${this.language['ONT configurations']} - ${this.language['Configuration']} - ${this.language['Services']} - ${this.language['Operations']}  - ${this.language['Calix Cloud']}`);
    ...
    if (environment.VALIDATE_SCOPE) {
      ...
      if (scopes && scopes['cloud.rbac.coc.services.configuration.ontconfigurations']?.length) {
        this.hasScopeAccess = true;
        if (scopes['cloud.rbac.coc.services.configuration.ontconfigurations'].indexOf('read') !== -1) this.scope['read'] = true;
        if (scopes['cloud.rbac.coc.services.configuration.ontconfigurations'].indexOf('write') !== -1) this.scope['write'] = true;
      }
    ...
    this.signupForm = new FormGroup({
      ...
      'provisionOntService': new FormControl(false),
      ...
      'showAsmInformation':new FormControl(false),
    ...
    if (this.signupForm.valid) {
      if (this.adminsData.length !== 0) {
        ...
        this.obj = {
          ...
          'rgManagement': 'NATIVE',
          'provisionOntService':this.provisionOntService,
          'showAsmInformation':this.showAsmInformation
        }
        let ontTimeError;
  ...
  changeOnttoSubscriber(checked: boolean) {
    this.closeModal();
    this.enableSubscriberAssocition = checked;
    this.save()
  }
  ...
  getData() {
    this.listSubs = this.service.getOrg(this.ORG_ID).subscribe((res: any) => {
      if (res) {
        this.orgData = res;
        if (this.orgData && this.orgData?.enableSubscriberAssoc != undefined) {
          this.enableSubscriberAssocition = this.orgData.enableSubscriberAssoc;
        } else {
          this.enableSubscriberAssocition = false;
        }
      } else {
        this.enableSubscriberAssocition = false;
      }
  ...
  confirmEnableSUbscriberChange(checked) {
    this.enableSubscriberAssoc = checked;
    this.enableSubscriberAssocition = checked;
    this.infoTitle = this.language['Confirm change :'];
    this.infoBody = this.language.enableDisableOntMapping;
    this.modalRef = this.dialogService.open(this.infoModal, { backdrop: 'static', keyboard: false, centered: true });
  }
  ...
  save() {
    ...
    const params = {
      "enableSubscriberAssoc": this.enableSubscriberAssoc
    };
