...
//import { CommonFunctionAllService } from '../../../app-services/common-function-all.service';
...
export class AddRoleComponent implements OnInit {
  ...
  levelThreeSelected: any = [];

  levelTwoSelectedData: string;
  levelThreeSelectedData: string;
  ...
  addRole() {
    ...
    this.loader = true;
    this.roleAddSubs = this.addRoleService.RoleAdd(params).subscribe((res: any) => {
      this.roleDatas = res;
  ...
  addOtherDetailsToRole(pdData) {
    ...
    const req = this.addRoleService.AddUsersByRoleId(users, this.roleDatas._id).forEach((resp: any) => {
      return resp;
    }).catch((error: any) => {
      return of(error);
    });
    ...
    if (pdData.length) {
      ...
      const req2 = this.addRoleService.AddRolePermissions(params, this.roleDatas._id).forEach((resp: any) => {
        return resp;
      }).catch((error: any) => {
        return of(error);
      });
  ...
  levelOneAction(scope: any, isChecked: any) {
    ...
    if (isChecked && !this.isExpanded.hasOwnProperty(scope._id)) this.expandCollapse(scope._id);
    else if (!isChecked && this.isExpanded.hasOwnProperty(scope._id)) this.expandCollapse(scope._id);
