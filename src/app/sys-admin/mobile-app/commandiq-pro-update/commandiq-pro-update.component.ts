...
@Component({
  selector: 'app-commandiq-pro-update',
  templateUrl: './commandiq-pro-update.component.html',
  styleUrls: ['./commandiq-pro-update.component.scss']
})
export class CommandiqProUpdateComponent implements OnInit {
  ...
  constructor(
    ...
    this.commonOrgService.currentPageAdder('commandWorx-update');
    ...
    this.titleService.setTitle(`CommandWorx - Mobile App - ${this.REDIRECT_URL === 'systemAdministration' ? 'System Administration' : 'Administration'} - Calix Cloud`);
  }
  ...
  getDetails() {
    ...
    this.service.commandIqList(this.SPID).toPromise().then((res: any) => {
      ...
      this.updateappIcon = res.body.whiteLogo;
      if (this.updateappIcon) {
        this.ISappIcon = true;
      } else {
        this.ISappIcon = false;
      }
      this.updatelogo = res.body.primaryLogo;
      if (this.updatelogo) {
        this.ISlogo = true;
      } else {
        this.ISlogo = false;
      }
      this.updateWhiteLogo = res.body.whiteLogo;
  ...
  add() {
    ...
    //uncoment
    if (this.updatelogo != '') {
      form_Data.append("primaryLogo", this.updatelogo);
    } else {
      ...
      if (logo.length) {
        ...
        $.each(logo, function (i, file) {
          form_Data.append("primaryLogo", file);
        });
      } else {
        form_Data.append("primaryLogo", '');
      }
    }
    ...
    if (this.updateWhiteLogo != '') {
      form_Data.append("whiteLogo", this.updateWhiteLogo);
    } else {
      ...
      if (white_logo.length) {
        ...
        $.each(white_logo, function (i, file) {
          form_Data.append("whiteLogo", file);
        });
      } else {
        form_Data.append("whiteLogo", '');
      }
    }
    ...
    this.hideError();
    this.loader = true;
    this.service.commandIqUpdate(form_Data).subscribe((jdata: any) => {
  ...
  goToWhiteLabelList() {
    ...
    if (this.isShad) {
      ...
      this.router.navigate([`/${this.REDIRECT_URL}/mobile-app/commandWorx`]);
    }
  }
  hideAppicon(): void {
    // this.ISappIcon = false;
