...
@Component({
  selector: 'app-commandiq-pro-create',
  templateUrl: './commandiq-pro-create.component.html',
  styleUrls: ['./commandiq-pro-create.component.scss']
})
export class CommandiqProCreateComponent implements OnInit {
  ...
  constructor(
    ...
    this.commonOrgService.currentPageAdder('commandWorx-create');
  ...
  add() {
    ...
    this.service.commandIqadd(form_Data).subscribe((jdata: any) => {
  ...
  cancel() {
    //this._location.back();
    if (this.isShad) {
      this.router.navigate([`/shad/commandWorx`]);
    } else {
