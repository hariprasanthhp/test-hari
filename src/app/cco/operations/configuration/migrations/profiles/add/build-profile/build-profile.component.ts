...
export class BuildProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.buildMigForm = new FormGroup({
      ...
      "dataProfile": new FormGroup({
        ...
        //   "downloadCIR": new FormControl('',Validators.pattern(/^([0-9]|[1-9]\d|[1-9]\d{2}|[1]\d{3}|20[0-3]\d|204[0-8])k$|^([1-9]|[1-9]\d|[1-9]\d{2}|[1-3]\d{3}|4000)m$/)),
        //   "uploadCIR": new FormControl('',Validators.pattern(/^([0-9]|[1-9]\d|[1-9]\d{2}|[1]\d{3}|20[0-3]\d|204[0-8])k$|^([1-9]|[1-9]\d|[1-9]\d{2}|[1-3]\d{3}|4000)m$/))
        // }),
    ...
    //, [Validators.pattern(/^[-+]?[\d]+$/), Validators.min(49152), Validators.max(65280)]
    //, [Validators.pattern(/^[-+]?[\d]+$/), Validators.min(0), Validators.max(65535)]
    //, [Validators.pattern(/^[a-z\d]([a-z\d\-]{0,61}[a-z\d])?(\.[a-z\d]([a-z\d\-]{0,61}[a-z\d])?)*$/i)]
    //, [Validators.pattern(/^[-+]?[\d]+$/), Validators.min(10), Validators.max(3600)]
    ...
    //, Validators.pattern(/^((([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])|(\w.*))$/gm)
    //, Validators.pattern(/^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/gm)
    //, Validators.pattern(/^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/gm)
    //, Validators.pattern(/^((([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])|.{0})$/gm)
  ...
  Initialise() {
    ...
    if (this.profiledata?.deviceType === 'RG' || this.deviceType === 'RG') {
...
"ServerIP": ''} })
    ...
    if(this.buildMigForm.value.serviceType==='DATA'){
      ...
      this.serviceProfileItems =this.serviceProfileItem.filter((el) => el.configurations.some(e=> e.category ==='Video Service'));
    }else{
      this.serviceProfileItems =this.serviceProfileItem.filter((el) => el.configurations.some(e=> e.category ==='Voice Service'));
    }
  }
  AutoPopulateServices(val){
    ...
    if(this.buildMigForm.value.serviceType==='DATA'){
      ...
      if(obj){
        ...
        // if(obj.parameterValues?.X_000631_MvrProfile){
        //   let bwObj =this.serviceProfileItem.filter((el) => el?._id ===obj.parameterValues?.X_000631_MvrProfile)
        //   obj.parameterValues.downloadCIR=bwObj[0].configurations[0].parameterValues.DownstreamCIR
        // obj.parameterValues.uploadCIR=bwObj[0].configurations[0].parameterValues.UpstreamCIR
        // }
        if(obj.parameterValues.VlanTagAction == false && obj.parameterValues.VLANID == '-1'){
  ...
  getCSCServiceProfileList(val?) {
    this.getServiceDefinition = this.service.GetCscService().subscribe((res: any) => {
      console.log("serviceProfileItem",res)
      if (res) {
  ...
  getServiceDefinitionList(val?) {
    ...
    this.getServiceDefinition = this.service.GetServiceDefinition(this.buildMigForm.value.serviceType).subscribe((res: any) => {
      if (res) {
        this.serviceDefinitionItems = res ? res : [];        
  ...
  dataModeSelect() {
    if (this.buildMigForm.value.dataProfile.ProductFamily == 'GigaCenter') {
      ...
      if(['v6','6rd'].includes(this.buildMigForm.get('dataProfile').value.version)) this.buildMigForm.get('dataProfile').patchValue({version:'v4'})
