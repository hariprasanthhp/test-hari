...
export class ListComponent implements OnInit {
  ...
  editProfile(data,type){
    let id = data?.ontSMP?.id ?data?.ontSMP?.id :data?.rgSMP?.id 
    this.router.navigate(['/cco/operations/configuration/migrations/profiles/add'], { queryParams:{smpId:id,type:type}});
 ...
 this.datatableElement.dtInstance.then((dtInstance: DataTables.Api) => {
  ...
  showdetails(data,type) {
   this.detail = data;
   let id=data?.ontSMP?.id ?data?.ontSMP?.id :data?.rgSMP?.id 
    this.router.navigate(['/cco/operations/configuration/migrations/profiles/detail'], { queryParams:{smpId:id,type:type}});
  }
  
  deleteProfile(data,type){
   this.deviceType=type
    this.smpId = data?.ontSMP?.id ?data?.ontSMP?.id :data?.rgSMP?.id 
    this.deletedata=data?.ontSMP ? data?.ontSMP:data?.rgSMP
  ...
  DeleteProfile(){
    this.loader=true
    this.service.DeleteProfile(this.smpId,this.sso.getOrgId(),this.deviceType).subscribe((res: any) => {
      this.success = true
      this.loader=false
      this.deletedata = "";
