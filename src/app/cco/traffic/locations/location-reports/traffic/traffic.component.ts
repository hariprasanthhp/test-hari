...
export class TrafficComponent implements OnInit {
  ...
  loadChartData(data: any) {
    ...
    this.subscribe = this.service.getTraffic(data).subscribe((res: any) => {
      ...
      if (data['criteriaSelected'] == 'usage') {
        this.trafficChartOptions = this.options.makeOptionsForNWTraffic(res, data, 'locations');
      } else {
        this.trafficChartOptions = this.options.makeOptionsForLineChart(res, data, 'locations');
      }
      ...
      if (data['criteriaSelected'] == 'usage') {
        this.trafficChartOptions = this.options.makeOptionsForNWTraffic([], data, 'locations');
      } else {
        this.trafficChartOptions = this.options.makeOptionsForLineChart([], data, 'locations');
      }
      if(document.getElementById('container')){
        this.Highcharts.chart('container', this.trafficChartOptions);
      }
  ...
  renderChart() {
    ...
    if (this.filters['criteriaSelected'] == 'usage') {
      this.trafficChartOptions = this.options.makeOptionsForNWTraffic(this.data, this.filters, 'locations');
    } else {
      this.trafficChartOptions = this.options.makeOptionsForLineChart(this.data, this.filters, 'locations');
    }
    if(document.getElementById('container')){
      this.Highcharts.chart('container', this.trafficChartOptions);
    }
