...
(function (Highcharts: any) {
  Highcharts.exportCharts = function (charts, options) {
    ...
    Highcharts.post(options.url, {
      filename: options.filename || 'Network-Top-Application-Traffic',
      type: options.type,
      scale: 1.9,
...
@Component({
  selector: 'app-application-traffic',
  templateUrl: './application-traffic.component.html',
  styleUrls: ['./application-traffic.component.scss']
})
export class ApplicationTrafficComponent implements OnInit {
  error: boolean;
  errorInfo: string = '';
  language: any;
  ...
  renderChart() {
    ...
    if (this.filters['rateSelected'] == 'Average') {
      upoptions = this.makeAreaChartOptions(this.data, 'usRate', 'Up', this.filters);
      downoptions = this.makeAreaChartOptions(this.data, 'dsRate', 'Down', this.filters);
    } else {
      this.filters['rateSelected'] = 'Max';
      upoptions = this.makeAreaChartOptions(this.data, 'peakUsRate', 'Up', this.filters);
      downoptions = this.makeAreaChartOptions(this.data, 'peakDsRate', 'Down', this.filters);
    }
    if(document.getElementById('container-up') && document.getElementById('container-up')){
      this.upChart = this.Highcharts.chart('container-up', upoptions);
      this.downChart = this.Highcharts.chart('container-down', downoptions);
  ...
  makeAreaChartOptions(res: any, type, rate, params: any): any {
    ...
    let options = {
      ...
      title: {
        text: rate === 'Down' ? '' : this.language['Network Top Application Traffic']
      },
      subtitle: {
    ...
    if (rate !== 'Down') {
      options['exporting'] = {
        filename: 'Network_Top_Application_Traffic',
        buttons: {
          contextButton: {
            menuItems: [
              {
                      ...
                      UpdataExport.push(
                        {
                          ...
                          'Usage(Byte)': res[i].usOctets ? res[i].usOctets : 0,
                          'Max Up Rate(bps)': res[i].peakUsRate ? res[i].peakUsRate : 0,
                          'Average Up Rate(bps)': res[i].usRate ? res[i].usRate : 0
                        }
                      )
                    }
                  }
                      ...
                      DowndataExport.push(
                        {
                          'Date Time': that.options.getDateTime(res[i].startPeriodSec, true, 'MM/dd HH:mm'),
                          'Name': res[i].name ? res[i].name : 'Other',
                          'Usage(Byte)': res[i].dsOctets ? res[i].dsOctets : 0,
                          'Max Down Rate(bps)': res[i].peakDsRate ? res[i].peakDsRate : 0,
                          'Average Down Rate(bps)': res[i].dsRate ? res[i].dsRate : 0
                        }
                      )
                    }
                  }
                  that.downLoadCSV('Network_Top_Application_Traffic', UpdataExport, DowndataExport, UpextraData, DownextraData);
                }
              }],
