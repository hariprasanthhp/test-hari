...
export class TopLocationsComponent implements OnInit {
  ...
  loadChartData(data: any) {
    ...
    this.subscribe = this.service.getTopLocations(data).subscribe((res: any) => {
      this.data = res;
      ...
      this.chartHeight = this.height.toString() + "px";
      setTimeout(() => {
        this.chartOptions = this.options.makeOptionsForLocations(res, 'bar', data);
      ...
      this.chartOptions = this.options.makeOptionsForLocations([], 'bar', data);
      if(document.getElementById('top-locations-container')){
        this.Highcharts.chart('top-locations-container', this.chartOptions);
      }
  ...
  renderChart() {
    this.chartOptions = null;
    this.chartOptions = this.options.makeOptionsForLocations(this.data, 'bar', this.filters);
    if(document.getElementById('top-locations-container')){
      this.Highcharts.chart('top-locations-container', this.chartOptions);
    }
  }
